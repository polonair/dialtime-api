...
class MasterController extends Controller
{
    ...
    private function getResult($request, $rq)
    {
        ...
        switch($request["action"])
        {
            ...
            case "route.reject": $result = $this->route_reject($request, $rq); break;
    ...
    private function route_reject($request, $rq)
    {
        if ($this->session !== null) 
        {
            $em = $this->get('doctrine')->getManager();
            ...
            $user = $this->session->getOwner();
            $master = $em->getRepository("ModelBundle:Master")->loadMasterByUser($user);
            $route = $em->getRepository("ModelBundle:Route")->loadOneForMaster($master, $request["data"]["route"]);

            $ticket = (new Ticket())
                ->setTheme("Отказ от звонка")
                ->setClient($user);
            $rejection = (new RouteRejection())
                ->setRoute($route)
                ->setReason($request["data"]["reason"])
                ->setState(RouteRejection::STATE_REJECTED_BY_MASTER)
                ->setPartnerTicket(null)
                ->setMasterTicket($ticket)
                ->setTransaction(null);
